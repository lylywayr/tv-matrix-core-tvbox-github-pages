...
def _parse_tvbox_json(text: str) -> ParsedContent:
    ...
    if isinstance(data, dict):
        for key in ("sites", "lives", "parses", "spider", "rules"):
            value = data.get(key)
            if isinstance(value, list):
                for entry in value:
                    if isinstance(entry, dict):
                        api = str(entry.get("api") or entry.get("url") or entry.get("ext") or "")
                        if api.startswith(("http://", "https://")) or key in {"sites", "lives"}:
    ...
    return ParsedContent(format=SourceFormat.TVBOX_JSON, valid_items=items, raw_title=_title_from_json(data))
...
def _parse_m3u(text: str) -> ParsedContent:
    ...
    for line in text.splitlines():
        ...
        if line.startswith("#EXTINF"):
            ...
            current_name = None
    return ParsedContent(format=SourceFormat.M3U, valid_items=items)


def _parse_txt(text: str) -> ParsedContent:
    items = [{"type": "url", "url": url} for url in discover_urls_from_text(text)]
    return ParsedContent(format=SourceFormat.TXT, valid_items=items)
