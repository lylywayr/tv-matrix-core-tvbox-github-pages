...
<body>
  ...
  <main class="wrap">
    ...
    <section class="grid">
      <table>
        <thead><tr><th>名称</th><th>健康</th><th>延迟</th><th>地址</th></tr></thead>
        <tbody id="rows"></tbody>
      </table>
  ...
  <script>
    ...
    fetch('data.json').then(r => r.json()).then(data => {
      ...
      rows.innerHTML = (data.sources || []).map(item => `<tr><td>${escapeHtml(item.name)}</td><td class="tag">${item.label} ${item.score}</td><td>${item.elapsed_ms || '-'}ms</td><td>${escapeHtml(item.url)}</td></tr>`).join('') || '<tr><td colspan="4">暂无可用线路</td></tr>';
